...
static long long A[matrix_size][matrix_size], B[matrix_size][matrix_size], C[matrix_size][matrix_size];
static unsigned lines_per_thread = matrix_size / n_workers;

long long* get_element(long long matr[matrix_size][matrix_size], unsigned y, unsigned x)
{
    return &matr[y][x];
}
